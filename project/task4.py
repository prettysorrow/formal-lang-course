...
def _build_product_adjacency(graph_desc, regex_desc):
    ...
    graph_num_states = graph_desc.num_states
    regex_num_states = regex_desc.num_states
    product_size = graph_num_states * regex_num_states
    rows, cols = [], []
    for label in set(graph_transitions_by_label) & set(regex_transitions_by_label):
        for graph_state_from, graph_state_to in graph_transitions_by_label[label]:
            for regex_state_from, regex_state_to in regex_transitions_by_label[label]:
                rows.append(graph_state_from * regex_num_states + regex_state_from)
                cols.append(graph_state_to * regex_num_states + regex_state_to)
    return csr_matrix(
